...
class SpaceSearcher
{
 ...
 public:
  ...
  // Use morton_pair constructor to create morton pair from tuple<code, Node>
  auto point2code = [&](const Point& pt) { return mc_.code(pt); };

  // Use transform iterator to create an iterator mapping Point -> Lambda
  auto code_iter_begin = thrust::make_transform_iterator(pfirst, point2code);
  auto code_iter_end = thrust::make_transform_iterator(plast, point2code);
  // Use the zip iterator<code, nodes>

  auto zip_iter_begin = thrust::make_zip_iterator(thrust::make_tuple(code_iter_begin, tfirst));
  auto zip_iter_end = thrust::make_zip_iterator(thrust::make_tuple(code_iter_end, tlast));

  auto tuple2pair = [](thrust::tuple<code_type, value_type> t) { return morton_pair(t); };

  auto morton_pair_begin = thrust::make_transform_iterator(zip_iter_begin, tuple2pair);
  auto morton_pair_end = thrust::make_transform_iterator(zip_iter_end, tuple2pair);

  // Construct z_data_!

  std::vector<morton_pair> tmp(morton_pair_begin, morton_pair_end);
  z_data_ = tmp;


  // Sort z_data_.
  std::sort(z_data_.begin(), z_data_.end(), [] (morton_pair a, morton_pair b) {
 ...
 private:
  ...
  struct morton_pair {
    ...
    morton_pair(thrust::tuple<code_type, value_type> t) {
      code_ = thrust::get<0>(t);
      value_ = thrust::get<1>(t);
    } 


    morton_pair(code_type code, value_type value) {
      code_ = code;
      value_ = value;
    }
  };
